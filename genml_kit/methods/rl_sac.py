...
class SACMethod(Method):
    ...
    def wire_data(self, args, pipeline):
        ...
        if hasattr(pipeline, "action_space") and hasattr(
            pipeline.action_space, "shape"
        ):
    ...
    def train_step(self, model, blob, global_step, *, labels=None):
        ...
        with torch.no_grad():
            next_action, _, next_log_prob, _, _ = model.actor.get_action_and_value(
                next_obs
            )
            # Target networks are plain Sequential modules; call forward directly
            q1_next = model.q1.target(
                torch.cat([next_obs, next_action], dim=-1)
            ).squeeze(-1)
            q2_next = model.q2.target(
                torch.cat([next_obs, next_action], dim=-1)
            ).squeeze(-1)
        ...
        if self._auto_alpha:
            ...
            alpha_loss = sac_alpha_loss(
                new_log_prob.detach(), self._target_entropy, alpha
            )
