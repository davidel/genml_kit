...
def build_parser():
    ...
    lora_group.add_argument(
        "--lora_alpha", type=float, default=16.0, help="LoRA alpha scaling."
    )
    lora_group.add_argument(
    ...
    parser.add_argument(
        "--image_size", type=int, default=448, help="Model input resolution."
    )
    parser.add_argument(
        "--batch_size", type=int, default=32, help="Training batch size per step."
    )
    parser.add_argument(
        "--log_every", type=int, default=20, help="Log training stats every N steps."
    )
    parser.add_argument(
...
def parse_args(argv=None):
    ...
    add_training_state_args(
        parser, state_save="opt,sched,amp", state_load="opt,sched,amp"
    )
    ...
    opt.add_argument(
        "--llrd_decay", type=float, default=0.0, help="Layer-wise LR decay factor."
    )
    opt.add_argument(
        "--lr_group",
        action=KVPairAction,
...
def main(argv=None):
    ...
    logging.info(
        "Resolved run: pipeline=%s method=%s (metric_key=%s)",
        pipeline.NAME,
        method.NAME,
        method.METRIC_KEY,
    )
    ...
    # Default no-op in DataPipeline; RL pipelines override.
...
def _post_train(args, pipeline, method, device, result):
    ...
    if result.interrupt_signals and result.interrupt_signals != ["SIGINT"]:
        logging.info(
            "Interrupted by %s; checkpoint saved, exiting.", result.interrupt_signals
        )
        return
